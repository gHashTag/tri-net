...
- **W2 FEC** ✅ Hamming(7,4) + **W2b bit-interleaver** (block 16cw=112b=14B; burst≤16 corrected, swept test). **W2 measurement (examples/fec_fer_bench) = HONEST NEGATIVE:** Hamming is NET-NEGATIVE at operating SNRs (σ=0.14 raw FER 3.2% vs FEC 6.5%) — 7/4 length + mis-correction outweigh the fix; only helps at the harsh tail. So **FEC is now OPT-IN (TRIOS_FEC=1), default OFF** — mesh never regresses. Interleaver correct+ready; **real win = stronger code Viterbi K=5** (viterbi_k5.t27). Commits iter3 a364818 + iterA c8c003d.
- **W3b real keys** ✅: StaticKey::generate/from_bytes/to_bytes; TRIOS_KEY secret file + `peer <id> <addr> <pubkey-hex>` config → real static-static session; falls back to demo key if unprovisioned. Proven on board 11 (public 5ee5c9…). **Closes threat #3.** Commit 78c394a. **W3 replay-resync (remaining):** needs on-air Noise-XX handshake (rebooted node reusing epoch/counter = nonce-reuse, not just replay → MUST re-key with fresh ephemeral). Handshake in crypto.rs ready; the on-air exchange over the lossy radio is the work.
