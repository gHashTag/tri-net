...
pub fn create_congestion_state(cwnd: u32, ssthresh: u32, state: u32, losses: u32) -> u32 {
    return (((((cwnd & 0xFF) << 24) | ((ssthresh & 0xFF) << 16)) | ((state & 0x3) << 14))
        | (losses & 0x3FFF));
}
...
pub fn find_congestion_controller(controllers: [u32; MAX_FLOWS as usize], flow_id: u32) -> u32 {
    let mut i: u32 = 0;
    while (i < MAX_FLOWS) {
        if (i == flow_id) {
            return i;
...
pub fn is_any_flow_congested(controllers: [u32; MAX_FLOWS as usize]) -> u32 {
    let mut i: u32 = 0;
    while (i < MAX_FLOWS) {
        if (is_congested(controllers[(i) as usize]) == 1) {
            return 1;
        }
        i = (i + 1);
    }
...
pub fn calculate_total_cwnd(controllers: [u32; MAX_FLOWS as usize]) -> u32 {
    let mut total: u32 = 0;
    let mut i: u32 = 0;
    while (i < MAX_FLOWS) {
        total = (total + get_cwnd(controllers[(i) as usize]));
        i = (i + 1);
    }
    return total;
}

pub fn allocate_fair_bandwidth(
    controllers: [u32; MAX_FLOWS as usize],
    total_bandwidth: u32,
) -> u32 {
    let mut active_flows: u32 = 0;
    let mut i: u32 = 0;
    while (i < MAX_FLOWS) {
        let cwnd: u32 = get_cwnd(controllers[(i) as usize]);
        if (cwnd > 0) {
