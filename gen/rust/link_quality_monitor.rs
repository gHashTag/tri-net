...
pub fn calculate_trend(history: [u8; 8]) -> i8 {
    let recent_avg: u8 = ((((((history[7] as u16) + (history[6] as u16)) + (history[5] as u16))
        + (history[4] as u16))
        >> 2) as u8);
    let older_avg: u8 = ((((((history[3] as u16) + (history[2] as u16)) + (history[1] as u16))
        + (history[0] as u16))
        >> 2) as u8);
...
pub fn is_degrading(current_etx: u8, trend: i8) -> bool {
    return ((current_etx > QUALITY_POOR) && (trend > (TREND_THRESHOLD as i8)));
}
