...
pub fn create_sim_event(event_id: u32, timestamp: u32, event_type: u32, node_id: u32) -> u32 {
    return (((((event_id & 0xFF) << 24) | ((timestamp & 0xFFFF) << 8))
        | ((event_type & 0xF) << 4))
        | (node_id & 0xF));
}
...
pub fn create_node_state(node_id: u32, status: u32, energy: u32, position: u32) -> u32 {
    return (((((node_id & 0xF) << 28) | ((status & 0xF) << 24)) | ((energy & 0xFF) << 16))
        | (position & 0xFFFF));
}
...
pub fn create_link_state(source: u32, dest: u32, quality: u32, latency: u32) -> u32 {
    return (((((source & 0xF) << 28) | ((dest & 0xF) << 24)) | ((quality & 0xFF) << 16))
        | (latency & 0xFFFF));
}
...
pub fn create_packet(packet_id: u32, source: u32, dest: u32, size: u32, sequence: u32) -> u32 {
    return ((((((packet_id & 0xFF) << 24) | ((source & 0xF) << 20)) | ((dest & 0xF) << 16))
        | ((size & 0xFF) << 8))
        | (sequence & 0xFF));
}
...
pub fn process_event(
    event: u32,
    node_states: [u32; MAX_NODES as usize],
    link_states: [u32; MAX_NODES as usize],
) -> u32 {
...
pub fn create_sim_stats(sent: u32, recv: u32, dropped: u32, latency: u32) -> u32 {
    return (((((sent & 0xFF) << 24) | ((recv & 0xFF) << 16)) | ((dropped & 0xFF) << 8))
        | (latency & 0xFF));
}
...
pub fn run_simulation_step(
    ...
    return create_sim_state(
        get_sim_time(new_state),
        (get_sim_event_count(new_state) - processed_count),
        get_sim_node_count(new_state),
    );
}
...
pub fn generate_simulation_report(stats: u32, duration: u32, node_count: u32) -> u32 {
    ...
    return (((((delivery_ratio & 0xFF) << 24) | ((avg_latency & 0xFF) << 16))
        | ((duration & 0xFF) << 8))
        | (node_count & 0xFF));
}
