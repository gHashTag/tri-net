...
pub fn create_proposal(proposal_id: u32, node_id: u32, value: u32, timestamp: u32) -> u32 {
    return (((((proposal_id & 0x3F) << 26) | ((node_id & 0x3F) << 20)) | ((value & 0xFF) << 12))
        | (timestamp & 0xFFF));
}
...
pub fn create_vote(node_id: u32, vote: u32, proposal_id: u32, timestamp: u32) -> u32 {
    return (((((node_id & 0xFF) << 24) | ((vote & 0x3) << 22)) | ((proposal_id & 0xFF) << 14))
        | (timestamp & 0x3FFF));
}
...
pub fn calculate_consensus_value(vote_array: [u32; 8], proposal_id: u32) -> u32 {
    let mut sum = 0;
    let mut count = 0;
    if ((get_proposal_node(get_vote(vote_array, 0)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 0)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 0)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 1)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 1)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 1)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 2)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 2)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 2)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 3)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 3)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 3)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 4)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 4)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 4)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 5)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 5)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 5)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 6)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 6)) != 0))
    {
        sum = (sum + get_proposal_value(get_vote(vote_array, 6)));
        count = (count + 1);
    }
    if ((get_proposal_node(get_vote(vote_array, 7)) == proposal_id)
        && (get_proposal_timestamp(get_vote(vote_array, 7)) != 0))
    {
