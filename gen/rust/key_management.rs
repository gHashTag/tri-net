...
pub fn create_key_entry(valid: u32, key_id: u32, key_value: u32, timestamp: u32) -> u32 {
    return (((((valid & 0x1) << 31) | ((key_id & 0x7F) << 24)) | ((key_value & 0xFFFF) << 8))
        | (timestamp & 0xFF));
}
...
pub fn find_key_by_id(store: [u32; 4], key_id: u32) -> u32 {
    if ((get_key_id(get_key_entry(store, 0)) == key_id)
        && (get_key_valid(get_key_entry(store, 0)) == KEY_VALID))
    {
        return 0;
    } else {
        if ((get_key_id(get_key_entry(store, 1)) == key_id)
            && (get_key_valid(get_key_entry(store, 1)) == KEY_VALID))
        {
            return 1;
        } else {
            if ((get_key_id(get_key_entry(store, 2)) == key_id)
                && (get_key_valid(get_key_entry(store, 2)) == KEY_VALID))
            {
                return 2;
            } else {
                if ((get_key_id(get_key_entry(store, 3)) == key_id)
                    && (get_key_valid(get_key_entry(store, 3)) == KEY_VALID))
                {
                    return 3;
                }
            }
...
pub fn add_key(store: [u32; 4], key_id: u32, key_value: u32, timestamp: u32) -> [u32; 4] {
    if (get_key_valid(get_key_entry(store, 0)) == KEY_INVALID) {
        return set_key_slot(
            store,
            0,
            create_key_entry(KEY_VALID, key_id, key_value, timestamp),
        );
    } else {
...
pub fn invalidate_key(store: [u32; 4], key_id: u32) -> [u32; 4] {
    ...
    if (index != 0xFF) {
        ...
        let new_entry = create_key_entry(
            KEY_INVALID,
            get_key_id(entry),
            get_key_value(entry),
