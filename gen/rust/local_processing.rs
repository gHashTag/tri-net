...
pub fn create_task(task_id: u32, priority: u32, size: u32, time: u32) -> u32 {
    return (((((task_id & 0xFF) << 24) | ((priority & 0x3) << 22)) | ((size & 0xFF) << 14))
        | (time & 0x3FFF));
}
...
pub fn create_result(task_id: u32, status: u32, size: u32, value: u32) -> u32 {
    return (((((task_id & 0xFF) << 24) | ((status & 0x3) << 22)) | ((size & 0xFF) << 14))
        | (value & 0x3FFF));
}
...
pub fn find_completed_result(
    results: [u32; MAX_RESULTS as usize],
    task_id: u32,
    count: u32,
) -> u32 {
    let mut i: u32 = 0;
...
pub fn create_resource_state(cpu: u32, memory: u32, tasks: u32, available: u32) -> u32 {
    return (((((cpu & 0xFF) << 24) | ((memory & 0xFF) << 16)) | ((tasks & 0xFF) << 8))
        | (available & 0xFF));
}
