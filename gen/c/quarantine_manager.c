...
uint32_t is_quarantined(uint32_t state) {
    uint32_t status = get_quarantine_status(state);
...
uint32_t quarantine_node(uint32_t state, uint32_t current_time) {
    ...
    uint32_t violations = get_violation_count(state);
...
uint32_t should_release_quarantine(uint32_t state, uint32_t current_time) {
    uint32_t status = get_quarantine_status(state);
    uint32_t start_time = get_start_time(state);
...
uint32_t record_violation(uint32_t state, uint32_t violation_record) {
    uint32_t node_id = get_quarantine_node_id(state);
    uint32_t violation_node_id = get_violation_node_id(violation_record);
    if ((node_id != violation_node_id)) {
        return state;
    }
    uint32_t current_violations = get_violation_count(state);
    uint32_t new_violations = (current_violations + 1);
    uint32_t node_id_ret = get_quarantine_node_id(state);
    uint32_t status = get_quarantine_status(state);
    uint32_t start_time = get_start_time(state);
...
uint32_t calculate_quarantine_severity(uint32_t state) {
    ...
    if ((status == STATUS_BANNED)) {
        ...
        uint32_t severity = (violations * 10);
        if ((severity > 50)) {
            return 50;
        } else {
            return severity;
