...
fn is_traffic_normal(stats: u32) bool {
    const total = calculate_total_traffic(stats);
    return (total >= TRAFFIC_LOW) and (total <= TRAFFIC_HIGH);
}
fn calculate_error_rate(stats: u32) u32 {
    const packets = get_packet_count(stats);
    const errors = get_error_count(stats);
    if (packets == 0) {
        return 0;
    }
...
fn detect_pattern(stats: u32, previous_stats: u32) u32 {
    const current_total = calculate_total_traffic(stats);
    const previous_total = calculate_total_traffic(previous_stats);
...
fn update_traffic(stats: u32, sent_add: u32, recv_add: u32, packets_add: u32, errors_add: u32) u32 {
    const sent = get_bytes_sent(stats);
    const recv = get_bytes_recv(stats);
    const packets = get_packet_count(stats);
    const errors = get_error_count(stats);
...
fn needs_attention(data: u64) bool {
    const pattern = get_analysis_pattern(data);
    const traffic = get_analysis_traffic(data);
    const stats = traffic;
    _ = stats; // dead after const-inlining
    return (pattern != PATTERN_NORMAL) or is_high_error_rate(traffic);
}
