...
use std::env;
use trios_mesh::rti::{KalmanTracker, LinkMeasurement, RtiNetwork, RtiTracker};
    ...
    for (id, ip) in &boards {
        ...
        if let Ok(o) = out {
            ...
            if let Ok(rssi_val) = s.parse::<f32>() {
                ...
                let dbm = -(rssi_val);
                eprintln!(
                    "  node {}: RSSI readback = {} → {:.0} dBm proxy",
                    id, rssi_val, dbm
                );
                ...
                for (other_id, _) in &boards {
                    if other_id != id {
                        measurements.push(LinkMeasurement {
                            from: *id,
                            to: *other_id,
                            rssi_dbm: dbm,
                            link_quality: 1.0,
...
fn run_scan_links() {
    ...
    for ip in ["192.168.1.11", "192.168.1.12", "192.168.1.13"] {
        ssh(
            ip,
            "echo 1 > /sys/bus/iio/devices/iio:device0/out_altvoltage1_TX_LO_powerdown 2>/dev/null",
        );
    }
    ...
    // Step 3: reconstruct.
    eprintln!(
        "\nReconstructing attenuation field from {} per-link measurements...",
        measurements.len()
    );
    let mut net = RtiNetwork::new(30, 30);
    net.add_node(1, 5.0, 5.0);
...
fn run_track() {
    ...
    for (frame, (ox, oy)) in path.iter().enumerate() {
        ...
        let links = [
            (1u32, 2u32, 5.0f32, 5.0, 25.0, 5.0),
            (1, 3, 5.0, 5.0, 15.0, 25.0),
            (2, 3, 25.0, 5.0, 15.0, 25.0),
        ];
...
fn run_multi() {
    ...
    eprintln!(
        "{} nodes, {} links",
        net.nodes.len(),
        net.nodes.len() * (net.nodes.len() - 1) / 2
    );
    // Calibrate: all links clean.
    let mut cal = Vec::new();
    for i in 0..net.nodes.len() {
        for j in (i + 1)..net.nodes.len() {
            cal.push(LinkMeasurement {
                from: net.nodes[i].id,
                to: net.nodes[j].id,
                rssi_dbm: -50.0,
                link_quality: 1.0,
    ...
    if let Some((cx, cy)) = img.centroid() {
        eprintln!(
            "FIX_RTI centroid: ({:.1}, {:.1}) — object localized",
            cx, cy
        );
    }
...
fn run_beamform() {
    ...
    for (a, b, atten) in &link_attens {
        ...
        eprintln!(
            ...
            qname,
            -(gain as i32),
            if boost == 1 { "yes" } else { "no" },
            if reroute == 1 { "yes" } else { "no" }
        );
    }
...
fn run_count() {
    ...
    for (i, (cx, cy, n, atten)) in blobs.iter().enumerate() {
        eprintln!(
            "  person {}: centroid ({:.0},{:.0}), {} pixels, atten {:.0} dB",
            i + 1,
            cx,
            cy,
            n,
    ...
    eprintln!(
        "\n{} people simulated → {} blobs detected",
        people.len(),
        blobs.len()
    );
...
fn run_fusion() {
    ...
    let blobs = img.detect_blobs(5.0, 3);
    eprintln!(
        "Detected {} object(s). Camera at node 1 (15,3), heading 0°.\n",
        blobs.len()
    );
    ...
    let cam_x = 15u32;
    let cam_y = 3u32;
    let cam_heading = 0u32;
    for (i, (bx, by, _, _)) in blobs.iter().enumerate() {
        let cmd =
            tri_video_fusion::fusion_command(cam_x, cam_y, cam_heading, *bx as u32, *by as u32);
        let slew = cmd & 0xFFF;
        let dir = (cmd >> 12) & 3;
        let should = (cmd >> 14) & 1;
...
fn run_daemon() {
    ...
    loop {
        ...
        eprintln!(
            "[daemon] blobs={}, covered={}({}%), score={}, tier={}",
            blobs.len(),
            covered,
            pct,
            score,
        ...
        if let Some(&(bx, by, _, ba)) = blobs.first() {
            if ba > 50.0 {
                eprintln!(
                    "[daemon] ⚠ strong blob at ({:.0},{:.0}) atten {:.0} — alert",
                    bx, by, ba
                );
...
fn run_dense() {
    ...
    eprintln!(
        "Deployment calibration: max detection range ≈ {} m",
        max_range
    );
    eprintln!(
        "Link margin at 30m: {} dB",
        tri_rfi_calib::link_margin_db(30)
    );
    eprintln!(
        "Nodes for 900m² area: {}\n",
...
fn run_record() {
    ...
    eprintln!(
        "Recording {} ticks to {} (CSV: tick,from,to,rssi)",
        max_ticks, file
    );
    let boards: Vec<(u32, &str)> = vec![
        (1, "192.168.1.11"),
...
fn run_replay() {
    ...
    eprintln!("═══ RTI Replay ═══");
    eprintln!(
        "Replaying {} through reconstruct → Kalman → blobs → coverage",
        file
    );
    let data = match std::fs::read_to_string(&file) {
        Ok(d) => d,
        Err(e) => {
            eprintln!("❌ read failed: {} (run 'tri-rti record' first)", e);
            return;
        }
    };
    // Parse CSV: tick,from,to,rssi_dbm. Group by tick.
    let mut frames: std::collections::BTreeMap<u32, Vec<LinkMeasurement>> =
    ...
    for line in data.lines().skip(1) {
        ...
        if parts.len() >= 4 {
            if let (Ok(tick), Ok(from), Ok(to), Ok(rssi)) = (
                ...
                frames.entry(tick).or_default().push(LinkMeasurement {
                    from,
                    to,
                    rssi_dbm: rssi,
                    link_quality: 1.0,
...
fn run_train() {
    ...
    for epoch in 0..epochs {
        ...
        for &(mx, mn, bl, cv, label) in &samples {
            ...
            let predicted = if score > tri_rti_ml::DECISION_THRESHOLD {
                1
            } else {
                0
            };
    ...
    eprintln!("\n✅ Training complete.");
    eprintln!(
        "Learned weights: w_max={}, w_mean={}, w_blob={}, w_cov={}",
        w_max, w_mean, w_blob, w_cov
    );
...
fn run_bench() {
    ...
    let _ = kf.update(
        img0.centroid().unwrap_or((0.0, 0.0)).0,
        img0.centroid().unwrap_or((0.0, 0.0)).1,
    );
    ...
    eprintln!(
        "  reconstruct (backprojection): {} μs ({:.2} ms)",
        recon_us,
        recon_us as f64 / 1000.0
    );
    eprintln!(
        "  full pipeline (recon+blobs+cov+kalman): {} μs ({:.2} ms)",
        full_us,
        full_us as f64 / 1000.0
    );
    eprintln!(
        "  Landweber (30 iters): {} μs ({:.2} ms)",
        lw_us,
        lw_us as f64 / 1000.0
    );
    eprintln!("");
    ...
    if full_us < target {
        eprintln!(
            "✅ Full pipeline {} μs < 100ms target — real-time capable",
            full_us
        );
    } else {
        eprintln!(
            "⚠ Full pipeline {} μs > 100ms target — needs optimization",
            full_us
        );
    }
