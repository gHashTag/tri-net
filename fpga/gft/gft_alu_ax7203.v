...
// clk_p on R4) buffered through an IBUFDS; the self-check drives two LEDs: led[0]=pass,
...
module gft_alu_ax7203 (
    input  wire       clk_p,   // 200 MHz DIFF_SSTL15 (+), package pin R4
    input  wire       clk_n,   // 200 MHz DIFF_SSTL15 (-), paired pin (verify vs board)
    input  wire       rst_n,   // active-low reset (a board key)
    output wire [3:0] led      // led[0]=pass, led[1]=fail
);
    wire clk;
`ifdef SIM
    assign clk = clk_p;        // behavioral buffer for iverilog
`else
    IBUFDS #(.DIFF_TERM("TRUE"), .IOSTANDARD("DIFF_SSTL15")) u_clk (.O(clk), .I(clk_p), .IB(clk_n));
`endif
    wire pass, fail;
