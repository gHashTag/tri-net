...
module tern_corr_pn_tree_tb;
    ...
    tern_corr_pn_tree #(.N(N),.W(W),.ACC(ACC),.AW(AW)) uut(.clk(clk),.rst(rst),
    ...
    initial begin
        ...
        if (peak==N*A) $display("TREE PN DESPREADER: autocorrelation peak on alignment -- OK");
        else           $display("TREE PN DESPREADER: FAILED");
        $finish;
    end
endmodule
