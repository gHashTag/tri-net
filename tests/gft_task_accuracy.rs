...
fn gft16_roundtrip(a: f64) -> f64 {
    ...
    if e < -GFT16_EMAX { e = -GFT16_EMAX; }
    if e > GFT16_EMAX { return f64::INFINITY; }
    let frac = a / 2f64.powi(e) - 1.0;
    let mut m = (frac * GFT16_MANT).round();
    let mut ee = e;
    if m >= GFT16_MANT { m = 0.0; ee += 1; }
...
fn binary16_roundtrip(a: f64) -> f64 {
    if a <= 0.0 { return 0.0; }
    if a > B16_MAX * (1.0 + 0.5 / B16_MANT) { return f64::INFINITY; }
...
fn rel(approx: f64, exact: f64) -> f64 {
    if !approx.is_finite() { return f64::INFINITY; }
...
fn measure(band: &str, lo: f64, hi: f64) -> (f64, f64, f64) {
    ...
    let fmt = |v: f64| if v.is_finite() { format!("{:>9.5}%", v * 100.0) } else { "  OVERFLOW".to_string() };
    println!(
        "  {:<8} band 2^[{:+.0},{:+.0}]   GF-T16 {}   binary16 {}   bf16 {}",
        band, lo, hi, fmt(g), fmt(b), fmt(f)
    );
    (g, b, f)
}
...
fn gft_task_level_accuracy() {
    ...
    assert!(g_narrow < f_narrow, "GF-T16 should beat bf16 on precision (narrow): {g_narrow} !< {f_narrow}");
    assert!(g_wide < f_wide, "GF-T16 should beat bf16 on precision (wide): {g_wide} !< {f_wide}");
    ...
    assert!(g_wide < b_wide, "GF-T16 should beat binary16 on the wide (range-bound) workload: {g_wide} !< {b_wide}");
    assert!(b_wide.is_infinite() || b_wide > 10.0 * g_wide, "wide binary16 error should be catastrophic vs GF-T16");
}
