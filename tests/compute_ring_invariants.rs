...
fn burned_total5_saturates_not_wraps() {
    ...
    assert_eq!(
        burned_total5(0x41, 0x42, 0x43, 0x44, 0x45, 1_100_000_000),
        4_294_967_295,
        "saturates"
    );
}
...
fn credit_adds_saturate_not_wrap() {
    ...
    assert_eq!(
        pool::balance_after_pool_settle(0xFFFF_FFF0, 1000, 300),
        0xFFFF_FFFF,
        "pool credit saturates"
    );
    assert_eq!(
        pool::balance_after_pool_settle(500, 1000, 300),
        800,
        "normal pool credit exact"
    );
    assert_eq!(
        bond::balance_after_resolve(0xFFFF_FFF0, 200, 0),
        0xFFFF_FFFF,
        "honest bond release saturates"
    );
    assert_eq!(
        bond::balance_after_resolve(0xFFFF_FFF0, 200, 1),
        0xFFFF_FFF0,
        "slash never adds"
    );
}
...
fn reputation_cap_survives_overflowing_gain() {
    ...
    assert_eq!(
        ...
        REP_MAX,
        "overflowing gain caps, not zeroes"
    );
}
...
fn every_resolver_is_stale_on_replay() {
    ...
    assert_eq!(
        resolve_full(5, 5, bin, bin, 0xAB, 0xAB, 0x9999, 0x4100),
        RESOLVE_STALE
    );
    assert_eq!(
        resolve_full_d256(5, 5, bin, bin, 1, 0x9999, 0x4100),
        RESOLVE_STALE
    );
    assert_eq!(
        resolve_bitnet_full(6, 6, gft, gft, 0xAB, 0xAB, 0x9999, 0x4100, 1),
        RESOLVE_STALE
    );
    assert_eq!(
        resolve_bitnet_quorum_full(
            6, 6, gft, gft, 0xAB, 0xAB, 0x9999, 0x4100, 0x4100, 0x9999, 1, 1, 1
        ),
        RESOLVE_STALE
    );
    assert_eq!(
        resolve_bitnet_quorum5_full(
            6, 6, gft, gft, 0xAB, 0xAB, 0x9999, 0x4100, 0x4100, 0x4100, 0x9999, 0x9999, 1, 1, 1, 1,
            1
        ),
        RESOLVE_STALE
    );
    assert_eq!(
        resolve_bitnet_d256_full(6, 6, gft, gft, 1, 0x9999, 0x4100, 1),
        RESOLVE_STALE
    );
}
...
fn every_resolver_is_family_mismatch_cross_family() {
    ...
    let (gft, bin) = (FMT_GFT, FMT_GF_BINARY);
    assert_eq!(
        resolve_full(0, 6, bin, gft, 0xAB, 0xAB, 0x9999, 0x4100),
        RESOLVE_FAMILY_MISMATCH
    );
    assert_eq!(
        resolve_bitnet_full(0, 6, gft, bin, 0xAB, 0xAB, 0x4100, 0x4100, 1),
        RESOLVE_FAMILY_MISMATCH
    );
    assert_eq!(
        resolve_bitnet_quorum_full(
            0, 6, gft, bin, 0xAB, 0xAB, 0x4100, 0x4100, 0x4100, 0x4100, 1, 1, 1
        ),
        RESOLVE_FAMILY_MISMATCH
    );
    assert_eq!(
        resolve_bitnet_d256_full(0, 6, gft, bin, 1, 0x4100, 0x4100, 1),
        RESOLVE_FAMILY_MISMATCH
    );
}
...
fn fresh_fraud_still_slashes() {
    ...
    let (gft, bin) = (FMT_GFT, FMT_GF_BINARY);
    assert_eq!(
        resolve_full(0, 6, bin, bin, 0xAB, 0xAB, 0x9999, 0x4100),
        RESOLVE_SLASH
    );
    assert_eq!(
        resolve_bitnet_full(0, 6, gft, gft, 0xAB, 0xAB, 0x9999, 0x4100, 1),
        RESOLVE_SLASH
    );
    assert_eq!(
        resolve_bitnet_d256_full(0, 6, gft, gft, 1, 0x9999, 0x4100, 1),
        RESOLVE_SLASH
    );
}
...
fn slashed_reward_never_finalizes() {
    ...
    assert_eq!(
        ...
        516,
        "clean reward finalizes after window"
    );
    assert_eq!(
        bal_after_finalize_checked(500, 16, 0, 100, 10, 1),
        500,
        "slashed never finalizes"
    );
    assert_eq!(
        bal_after_finalize_checked(500, 16, 0, 999_999, 10, 1),
        500,
        "slashed blocked no matter the clock"
    );
}
...
fn clawback_removes_reward_from_pending() {
    ...
    assert_eq!(pend0, 16);
    assert_eq!(
        pending_after_release(pend0, 16),
        0,
        "clawback empties pending"
    );
    assert_eq!(
...
fn width_binding_closes_precision_downgrade() {
    ...
    assert_eq!(
        ...
        skill_op(SKILL_GFT8_MUL),
        "GFT16 and GFT8 share op"
    );
    assert_eq!(
        skill_family(SKILL_GFT16_MUL),
        skill_family(SKILL_GFT8_MUL),
        "GFT16 and GFT8 share family"
    );
    // Binding layer: the bare gate binds a GFT8 receipt to a GFT16 assignment; sized rejects.
    assert!(
        result_binds_assign(0x777, 0x777, 0x777, SKILL_GFT16_MUL, FMT_GFT, 0x11),
        "bare gate binds on family+op"
    );
    assert!(
        !result_binds_assign_sized(0x777, 0x777, 0x777, SKILL_GFT16_MUL, FMT_GFT, 0x11, 8),
        "sized rejects a GFT8-width receipt for a GFT16 assignment"
    );
    assert!(
        result_binds_assign_sized(0x777, 0x777, 0x777, SKILL_GFT16_MUL, FMT_GFT, 0x11, 16),
        "matching width binds"
    );
    ...
    assert!(
        ...
        "width-blind full gate admits the downgrade"
    );
    assert!(
        !admit_result_signed_sized(
            0x777,
            0x777,
            0x777,
            SKILL_GFT16_MUL,
        ...
        "sized full gate rejects the GFT8 downgrade"
    );
    assert!(
        admit_result_signed_sized(
            0x777,
        ...
        "sized full gate admits the matching width"
    );
    ...
    assert!(
        ...
        "right width but stale still rejects"
    );
}
...
fn challenger_stake_griefing_vs_nonfault() {
    ...
    assert_eq!(
        ...
        0,
        "malformed griefing burns stake"
    );
    assert_eq!(
        challenger_stake_after_bound(50, RESOLVE_FAMILY_MISMATCH),
        0,
        "family-confusion griefing burns stake"
    );
}
...
fn verifier_accountability() {
    ...
    assert_eq!(
        verifier_stake_after(50, 0),
        50,
        "agreeing verifier keeps stake"
    );
...
fn gft_validity_and_finiteness() {
    ...
    assert!(
        is_valid_gft(79, 4),
        "offset just below the special row valid"
    );
    assert!(
        !is_valid_gft(80, 4),
        "the special row (offset_max) is not a valid value"
    );
...
fn bitnet_is_active_treats_0b11_as_inactive() {
    ...
    assert_eq!(
        is_active(3),
        0,
        "0b11 -> inactive, not a spurious active weight"
    );
}
...
fn gft_payability_rejects_out_of_range() {
    ...
    assert_eq!(
        ...
        0,
        "special row withheld"
    );
    assert_eq!(
        payable_flag(FMT_GFT, 81, 0, 0, 0, 80),
        0,
        "just past the ladder withheld"
    );
    assert_eq!(
        payable_flag(FMT_GFT, 100, 0, 0, 0, 80),
        0,
        "out-of-range withheld (was the garbage-pay hole)"
    );
    assert_eq!(
        payable_flag(FMT_GFT, u32::MAX, 0, 0, 0, 80),
        0,
        "max-u32 garbage withheld"
    );
    // and settle actually withholds the reward for the garbage offset.
    assert_eq!(
        settle_checked_gft(100, 16, 1, 100, 80),
        100,
        "out-of-range GF-T settles nothing"
    );
    assert_eq!(
        settle_checked_gft(100, 16, 1, 40, 80),
        116,
        "in-range GF-T still pays"
    );
}
...
fn gft_payability_matches_validity() {
    ...
    for off in [0u32, 1, 40, 79, 80, 81, 100, 1000, u32::MAX] {
        ...
        assert_eq!(
            payable, valid,
            "payable_flag must equal is_valid_gft for offset {off}"
        );
    }
}
...
fn binary_payability_withholds_only_inf_nan() {
    ...
    assert_eq!(
        ...
        1,
        "GF16 finite value payable"
    );
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x0000, 6, 9, 1, 0),
        1,
        "GF16 zero (exp 0) is finite -> payable"
    );
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x0001, 6, 9, 1, 0),
        1,
        "GF16 subnormal (exp 0, mant!=0) payable"
    );
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x7E00, 6, 9, 1, 0),
        0,
        "GF16 +Inf (exp all-ones, mant 0) withheld"
    );
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x7E01, 6, 9, 1, 0),
        0,
        "GF16 NaN (exp all-ones, mant!=0) withheld"
    );
    // A format WITHOUT Inf/NaN (has_inf 0, e.g. GF8): every exponent is a normal value.
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x70, 3, 4, 0, 0),
        1,
        "GF8 max-exp with no Inf format -> normal, payable"
    );
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x70, 3, 4, 1, 0),
        0,
        "same pattern WITH has_inf -> withheld"
    );
    ...
    assert_eq!(
        payable_flag(FMT_GF_BINARY, 0x4200, 6, 9, 1, 0),
        payable_flag(FMT_GF_BINARY, 0x4200 | (1 << 20), 6, 9, 1, 0),
        "a high garbage bit does not change the exp-field verdict"
    );
}
